...
<template>
    ...
    <section v-if="manager.activeTab === 'workspace'" class="chat-panel">
        ...
        <div v-else class="messages">
            ...
            <div v-if="manager.isChatting && manager.selectedAgent && manager.selectedAgent" class="typing">{{ manager.selectedAgent.name }} is thinking...</div>
        </div> -->
        <form v-if="manager.selectedAgent && manager.selectedAgent" class="composer" @submit.prevent="manager.sendMessage"><textarea v-model="manager.messageInput" rows="1"
    ...
    <section v-else class="knowledge-panel">
        <div class="section-intro"><span class="eyebrow">CONTEXT LIBRARY</span>
            <h2 v-if="manager.selectedAgent && manager.selectedAgent">What {{ manager.selectedAgent.name }} knows</h2>
        ...
        <div v-else class="source-empty">No knowledge sources yet. <button v-if="manager.selectedAgent && manager.selectedAgent" @click="manager.editAgent(manager.selectedAgent)">Add one
