...
<template>
    <form class="editor-modal" @submit.prevent="manager.saveAgent">
        <div class="modal-heading">
            <div><span class="eyebrow">{{ manager.isEditing ? 'EDIT AGENT' : 'NEW AGENT' }}</span>
        ...
        </div><label>Name<input v-model="manager.formData.name" required
                placeholder="e.g. Editorial partner"></label><label>Purpose<textarea v-model="manager.formData.purpose" required
                rows="2"
                ...
                class="label-hint">one per line</span><textarea v-model="manager.formData.knowledge" rows="4"
