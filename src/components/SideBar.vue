...
<template>
    ...
    <div v-else-if="!manager.agents.length" class="empty-state">Your first agent starts here.</div><button
        v-for="agent in manager.agents" :key="agent.id" class="agent-row"
        :class="{ active: manager.isThisSelectedAgent(agent) }" @click="manager.selectAgent(agent)"><span
            class="agent-icon">{{ agent.name.slice(0, 1).toUpperCase()
